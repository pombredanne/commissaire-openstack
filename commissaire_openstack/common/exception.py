...
"""Commissaire_Openstack base exception handling.

Includes decorator for re-raising Commissaire_Openstack-type exceptions.
...
from commissaire_openstack.common.i18n import _LE
...
try:
    ...
    # Note:work around for commissaire_openstack run against master branch
    # in devstack gate job, as commissaire_openstack not branched yet
...
def wrap_controller_exception(func, func_server_error, func_client_error):
    ...
    - if an unhandled Exception or a Commissaire_OpenstackException with an error code >=500
    is catched, raise a http 5xx ClientSideError and correlates it with a log
    message

    - if a Commissaire_OpenstackException is catched and its error code is <500, raise a http
    ...
    def wrapped(*args, **kw):
        try:
            ...
            if isinstance(excp, Commissaire_OpenstackException):
...
def wrap_keystone_exception(func):
    """Wrap keystone exceptions and throw Commissaire_Openstack specific exceptions."""
    @functools.wraps(func)
    def wrapped(*args, **kw):
        try:
            return func(*args, **kw)
...
class ObjectNotFound(Commissaire_OpenstackException):
    message = _("The %(name)s %(id)s could not be found.")


class ObjectNotUnique(Commissaire_OpenstackException):
...
class AuthorizationFailure(Commissaire_OpenstackException):
    message = _("%(client)s connection failed. %(message)s")


class UnsupportedObjectError(Commissaire_OpenstackException):
    message = _('Unsupported object type %(objtype)s')


class IncompatibleObjectVersion(Commissaire_OpenstackException):
    message = _('Version %(objver)s of %(objname)s is not supported')


class OrphanedObjectError(Commissaire_OpenstackException):
    message = _('Cannot call %(method)s on orphaned %(objtype)s object')


class Invalid(Commissaire_OpenstackException):
...
class GetDiscoveryUrlFailed(Commissaire_OpenstackException):
