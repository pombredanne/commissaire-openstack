...
echo_summary "Commissaire_Openstack's post_test_hook.sh was called..."
(set -o posix; set)
...
export ZUN_DIR="$BASE/new/commissaire_openstack"
sudo chown -R jenkins:stack $ZUN_DIR

# Use tempest to test commissaire_openstack api service

# Import devstack functions 'iniset', 'iniget' and 'trueorfalse'
source $BASE/new/devstack/functions
echo "TEMPEST_SERVICES+=,commissaire_openstack" >> $localrc_path
...
sudo -H -u jenkins tox -eall-plugin -- commissaire_openstack.tests.tempest.api --concurrency=1
