...
from commissaire_openstack.tests.unit.api import base as api_base
...
class TestRootController(api_base.FunctionalTest):
    def setUp(self):
        ...
        self.root_expected = {
            ...
            u'description': u'Commissaire_Openstack is an OpenStack project which '
        ...
        self.v1_expected = {
            ...
            [{u'base': u'application/json',
              u'type': u'application/vnd.openstack.commissaire_openstack.v1+json'}],
            u'links': [{u'href': u'http://localhost/v1/',
                        u'rel': u'self'},
                       {u'href':
                        u'http://docs.openstack.org/developer'
                        '/commissaire_openstack/dev/api-spec-v1.html',
    ...
    def test_noauth(self):
        # Don't need to auth
        paste_file = "commissaire_openstack/tests/unit/api/controllers/noauth-paste.ini"
        ...
        response = app.get('/v1/services/')
        self.assertEqual(200, response.status_int)

    def test_auth_with_no_public_routes(self):
        # All apis need auth when access
        paste_file = "commissaire_openstack/tests/unit/api/controllers/auth-paste.ini"
        app = self.make_app(paste_file)
    ...
    def test_auth_with_root_access(self):
        ...
        paste_file = "commissaire_openstack/tests/unit/api/controllers/auth-root-access.ini"
        app = self.make_app(paste_file)
        ...
        response = app.get('/v1/services', expect_errors=True)
        self.assertEqual(401, response.status_int)

    def test_auth_with_v1_access(self):
        # Only /v1 can access without auth
        paste_file = "commissaire_openstack/tests/unit/api/controllers/auth-v1-access.ini"
        app = self.make_app(paste_file)
